...
require_once dirname( __FILE__ ) . '/JSLikeHTMLElementForNerd.php';

// Alternative usage (for testing only!)
// uncomment the lines below and call ReadabilityForNerd.phphp in your browser
// passing it the URL of the page you'd like content from, e.g.:
// ReadabilityForNerd.phphp?url=http://medialens.org/alerts/09/090615_the_guardian_climate.php
/*
if (!isset($_GET['url']) || $_GET['url'] == '') {
	die('Please pass a URL to the script. E.g. ReadabilityForNerdrd.php?url=bla.com/story.html');
}
$url = $_GET['url'];
if (!preg_match('!^https?://!i', $url)) $url = 'http://'.$url;
$html = file_get_contents($url);
$r = new ReadabilityForNerd($html, $url);
$r->init();
echo $r->articleContent->innerHTML;
*/

class ReadabilityForNerd {
	...
	/**
	 * Create instance of ReadabilityForNerd.
	 *
	...
	public function init() {
		...
		if ( ! $articleContent ) {
			...
			$articleContent->innerHTML = '<p>Sorry, ReadabilityForNerd was unable to parse this page for content.</p>';
		}
